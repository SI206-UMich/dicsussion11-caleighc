...
def create_patients_table(cur, conn):
    ...
    # Pet id, name (string), species_id (number), age (integer), cuteness (integer),
    # aggressiveness (number)
    cur.execute("CREATE TABLE Patients (pet_id INTEGER PRIMARY KEY, name TEXT, species_id INTEGER, age INTEGER, cuteness INTEGER, \
...
def add_pets_from_json(filename, cur, conn):
    ...
    for i in json_data:
        ...
        cur.execute("INSERT INTO Patients (pet_id,name,species_id,age,cuteness,aggressiveness) \
            VALUES (?,?,?,?,?,?)",(pet_id,name,species_id,age,cuteness,aggressiveness))
        count += 1
    conn.commit()
